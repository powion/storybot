...
class StoryGen:
    def __init__(self, shortname, min_grams=2, max_grams=5):
        ...
        # Split into sentences and put to lowercase
        tokenized_sentences = nltk.tokenize.sent_tokenize(raw)
        ...
        for n in range(min_grams, max_grams + 1):
            ngrams = [nltk.ngrams(sentence, n, pad_left=True, left_pad_symbol="<s>") for sentence in tokenized_sentences]
            ...
            self.estimates[n] = nltk.ConditionalProbDist(dist,
                                                         smoothing_factory, bins=len(self.vocabulary))  # TODO: param bins: The number of possible sample outcomes.
    ...
    def nextInstance(self):
        ...
        out = out = self.startPhrase[:] if self.sentence_count == 0 else [["<s>", "NN"]]
        ...
        while not self.isTerminator(out[-1][0], out[-1][1]):
            ...
            num_prev = len(out)
            if VERBOSE:
                print("Length: ", len(out), file=sys.stderr)
            ...
            for grams in range(max_grams, min_grams - 1, -1):
                ...
                if VERBOSE:
                    print("Most common: ", most_common, file=sys.stderr)
