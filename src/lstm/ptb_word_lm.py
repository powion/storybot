...
class Lstm:
    ...
    def words2int(self, words):
        # Replace all words missing in the dictionary with the code of the <unk> word.
        unk_int = self.word2id["<unk>"]
        return [self.word2id.get(word, unk_int) for word in words]
    ...
    def predict_raw(self, word_list):
        ...
        with session.as_default():
            ...
            x = data
            y = data
            logits, state, _ = session.run([m.logits, m.final_state, tf.no_op()],
                                         {m.input_data: x,
                                          m.targets: y,
                                          m.initial_state: state})
        return logits
